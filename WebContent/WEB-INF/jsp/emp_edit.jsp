...
<body>
	<h2>社員データを修正します</h2>
	<form action="./EmployeeDatabase" method = "post" enctype = "multipart/form-date">
		...
		<p>住所：<input type = "text" name = "dateEntering" placeholder = "YYYY-MM-DD" value = "${ empEdited.dateEntering }"></p>
		<p>所属：
			<select name = "deptID">
				<c:forEach var = "dept" items = "${ deptAllList }">
					<option value ="${ dept.deptID }"
						<c:if test = "${ empEdited.deptID == dept.deptID }">
							selected
						</c:if>>
						${ dept.deptName }
				</c:forEach>
